...
public class WorldPlot
{
	...
	Position FindClosestMove(IUnit unit, TactikonState state, AIInfo info)
	{
		...
		int dangerMap[][] = null;
		if (unit.GetDomain() == Domain.Air) dangerMap = info.airDangerMap;
		if (unit.GetDomain() == Domain.Land) dangerMap = info.landDangerMap;
		if (unit.GetDomain() == Domain.Water) dangerMap = info.seaDangerMap;
		...
		for (Position move : possibleMoves)
		{
			...
			{
				if (dangerMap[move.x][move.y] < bestDanger)
				{
					bestMove = move;
					bestDanger = dangerMap[move.x][move.y];
				}
			}
		}
