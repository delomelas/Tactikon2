...
public class AIInfo
{
	...
	public void AddDanger(IUnit unit, int x, int y, TactikonState state)
	{
		IUnit airUnit = new UnitFighter();
		IUnit landUnit = new UnitInfantry();
		IUnit seaUnit = new UnitBoatTransport();
		if (x >= 0 && x < state.mapSize && y >= 0 && y < state.mapSize)
		{
		    airDangerMap[x][y] = airDangerMap[x][y] + unit.GetAttack(airUnit, state);
		    landDangerMap[x][y] = landDangerMap[x][y] + unit.GetAttack(landUnit, state);
		    seaDangerMap[x][y] = seaDangerMap[x][y] + unit.GetAttack(seaUnit, state);
	...
	{
		ArrayList<Position> positions = state.GetPotentialMoves(unit.mUnitId);
		ArrayList<Position> attackPositions = new ArrayList<Position>();
		
		for (Position pos : positions)
		{
			for (int xx = pos.x - (unit.GetAttackRange() ); xx <= pos.x + (unit.GetAttackRange()); ++xx)
			{
				for (int yy = pos.y - (unit.GetAttackRange()); yy <= pos.y + (unit.GetAttackRange()); ++yy)
				{
					int dist = Math.abs(pos.x - xx) + Math.abs(pos.y - yy);
					if (dist <= unit.GetAttackRange())
					{
						boolean got = false;
						for (Position bob : attackPositions)
						{
						...
						if (got == false)
						{
							attackPositions.add(new Position(xx, yy));
						}
		...
		{
			AddDanger(unit, pos.x, pos.y, state);
		}
	...
	{
		airDangerMap = new int[state.mapSize][state.mapSize];
		seaDangerMap = new int[state.mapSize][state.mapSize];
		landDangerMap = new int[state.mapSize][state.mapSize];
